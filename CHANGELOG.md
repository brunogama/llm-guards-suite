...
### Fixed
- APIGuard now compatible with Swift 6.0 symbol graph API
  - Removed unsupported `--target` and `--output-dir` flags from `swift package dump-symbol-graph`
  - Now correctly reads from `.build/symbol-graphs/` directory
  - Matches target-specific symbol graph files by filename pattern
